	...
	<div class="page-heading">
		...
		<section class="section">
			<div class="card">
				...
				@endif
				<form action="{{ route('task.update', $task) }}" method="post" class="card-body">
					@csrf
					@method('PUT')
					...
					<div class="mb-2">
						...
						<input type="text" class="form-control @error('title') is-invalid @enderror" value="{{ old('title', $task->title) }}" name="title" id="title" required>
						@error('title')
					...
					<div class="mb-2">
						...
						<input type="datetime-local" value="{{ old('due_date', $task->due_date) }}" class="form-control @error('due_date') is-invalid @enderror date" name="due_date" id="due_date" required>
						@error('due_date')
							<div class="invalid-feedback">{{ $message }}</div>
						@enderror
					</div>
					...
					<div class="mb-2">
						...
						<textarea class="form-control @error('description') is-invalid @enderror" name="description" id="description" required>{{ old('description', $task->description) }}</textarea>
						@error('description')
							<div class="invalid-feedback">{{ $message }}</div>
						@enderror
					</div>

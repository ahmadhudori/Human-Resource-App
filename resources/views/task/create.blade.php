				...
				<form action="{{ route('task.store') }}" method="post" class="card-body">
					...
					<div class="mb-2">
						...
						<select name="assigned_to" class="form-control text-gray-600 @error('assigned_to') is-invalid @enderror" id="assigned_to">
							...
							@foreach ($employees as $employee)
								<option value="{{ $employee->id }}" @if(old('assigned_to') == $employee->id) selected @endif>{{ $employee->fullname }}</option>
							@endforeach
						</select>
						@error('assigned_to')
							<div class="invalid-feedback">{{ $message }}</div>
					...
					<div class="mb-2">
						...
						<select name="status" class="form-control @error('status') is-invalid @enderror" id="status">
							...
							<option value="onProgress" @if (old('status') == 'onProgress') selected @endif>On Progress</option>
